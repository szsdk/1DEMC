...
from emc_core import *
from emc_tool import *
...
group = parser.add_mutually_exclusive_group()
group.add_argument("--show", help="plot iterations", action="store_true")
group.add_argument("--movie", help="save iteration process to a movie", action="store_const", const="./recon.mp4") 
...
def get_iter(r):
    ...
    i = start
    prob = np.random.rand(*patterns.shape)
    while True:
        recon_next, prob_next = EMC(recon, patterns, fluence)
        logging.info("%07dth iter, recon change: %.3E, orien change: %.3E", 
                i, np.mean((recon_next-recon)**2),
                np.sum(
                    np.abs(
                        np.argmax(prob_next, axis=1)-np.argmax(prob, axis=1)))/prob.shape[1]
                )
        recon, prob = recon_next, prob_next
        i += 1
        yield recon, prob
...
def init_intens_plot():
    ...
    global line, example_patterns, prob_array
    prob_array = ax[1].imshow(np.random.rand(S,N), animated=True)
    ax[1].set_xlabel("patterns")
    ax[1].set_ylabel("orientations")
    ax[1].set_title("probabilities for each pattern in each oritation")
    ax[0].plot(true_intens, label="true intensity")
    line, = ax[0].plot(recon, label="reconstructed intensity")
...
if args.show or args.movie:
    fig, ax = plt.subplots(2,1,figsize=(10,6))
    fig.canvas.mpl_connect('button_press_event', onClick)
    n_sample = 5
    anim = animation.FuncAnimation(fig, update_intens_plot,frames=itertools.islice(get_iter(recon),args.iteration), init_func=init_intens_plot, interval=15)
    if args.movie:
        anim.save(args.movie, fps=1.0, dpi=200)
    else: 
        plt.show()
else:
